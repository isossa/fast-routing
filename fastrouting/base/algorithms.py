...
from fastrouting.utils.route import node_is_active, add_link, set_node_status, get_link_status, is_interior, \
    get_route_demand, get_route_distance
from fastrouting.utils.savingsutils import get_maximum_savings
from fastrouting.utils.route import get_route_demand
...
def get_driver_availability(driver: str, link: tuple, availability_map: dict) -> bool:
    if driver in availability_map.keys():
        ...
        i, j = link
        return (availability[i - 1] == 1) and (availability[j - 1] == 1)
    else:
        return False


def get_availability_score(availability: list) -> dict:
    """Compute availability score and sort it in decreasing order

    :param availability:
    :return:
    """
    scores = {'D' + str(driver + 1): sum(row) for driver, row in enumerate(availability)}
    scores = dict(sorted(scores.items(), key=lambda item: item[1], reverse=True))
    return scores


def get_availability_map(availability: list) -> dict:
...
def insert_on_margin(routes: dict, sorted_savings: dict, capacity: int, demand: dict, customers: dict,
    ...
    for location in remaining_locations:
        ...
        route = update_route(link, route, relaxed_capacity, demand, customers)
...
def insert_last_customer(routes: dict, savings: dict, capacity: int, demand: dict, customers: dict):
